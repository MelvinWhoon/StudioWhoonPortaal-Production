    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
          <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-4">Totale Omzet</span>
          <div className="flex items-end justify-between">
            <span className="text-3xl font-black text-slate-900 tracking-tighter">€{stats.financials?.totalRevenue?.toLocaleString('nl-NL') || 0}</span>
            <span className="text-2xl opacity-20">💰</span>
          </div>
        </div>
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
          <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-4">Totaal Ontvangen</span>
          <div className="flex items-end justify-between">
            <span className="text-3xl font-black text-green-600 tracking-tighter">€{stats.financials?.totalPaid?.toLocaleString('nl-NL') || 0}</span>
            <span className="text-2xl opacity-20">✅</span>
          </div>
        </div>
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
          <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-4">Totaal Openstaand</span>
          <div className="flex items-end justify-between">
            <span className="text-3xl font-black text-orange-500 tracking-tighter">€{stats.financials?.totalOutstanding?.toLocaleString('nl-NL') || 0}</span>
            <span className="text-2xl opacity-20">⏳</span>
          </div>
        </div>
      </div>
