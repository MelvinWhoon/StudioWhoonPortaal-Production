...
const Sidebar: React.FC = () => {
  ...
  const getMenuItems = () => {
    if (user?.role === UserRole.SUPER_ADMIN) {
      return [
        ...
        { label: 'Demo Links', trans: 'Demo Links', icon: <Icons.Link /> },
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full text-slate-900 shadow-2xl overflow-hidden" style={{ backgroundColor: activeProject?.sidebarColor || '#B7A996' }}>
      <div className="p-10 flex flex-col items-center gap-6 border-b border-black/5">
        <div className="w-24 h-24 bg-white/40 p-3 rounded-[2.5rem] backdrop-blur-md shadow-inner flex items-center justify-center">
           <img src={activeProject?.logoUrl || IMAGES.LOGO} alt="Portal Logo" className="w-full h-full object-contain" />
        </div>
